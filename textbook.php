...
kogus, riiul, marksona, markused FROM textbook order by pealkiri LIMIT $start_from, $record_per_page" ;
...
<div class= "menu">
  ...
  <table  id="table1">
    <tr>
		...
		<th onclick="sortTable(1)">Õp klass</th>  
...
if (isset($_POST['column'])){
	...
	$sql= "SELECT id, klass, pealkiri, autor, aasta, liik, keel, valjaandja, 
	kogus, riiul, marksona, markused FROM textbook where  $column like '%$search%'" ;}
$result = mysqli_query($conn, $sql) or die("error:".mysqli_error($conn));
}
    while($row = mysqli_fetch_array($result)) {
        ...
        echo "<td><a href='textbook_edit.php?id=$row[id]' style='text-decoration: none'>Muuda</a> </td>";
		echo "<td> <a href='textbook_delete.php?id=$row[id]'style='text-decoration: none' class='delete' >Kustuta</a></td></tr>";
}
    ?>
...
<?php
	...
	keel, valjaandja, kogus, riiul, marksona, markused FROM textbook";
    ...
    if($page > 1)
    {
     echo "<a href='textbook.php?page=1'>Tagasi</a>";
    ...
    {
     echo "<a href='textbook.php?page=".($page + 1)."'>>></a>";
     echo "<a href='tekstbook.php?page=".$total_pages."'>Edasi</a>";
    }
  ?>  
</div>
