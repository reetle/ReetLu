...
$result = mysqli_query($conn, $sql) or die("error:".mysqli_error($conn));
  ...
  <tr>
	...
	<td>'.$row["marksona"].'</td>
	<td>'.$row["markused"].'</td>	
	<td><a href="borrow_meth_library.php?id='.$row["id"].'" class="btn  btn-sm">Laenuta</a></td>
	<td><a href="write_off_meth_library.php?id='.$row["id"].' "class="btn btn-sm">Kanna Maha</i></a></td>
  </tr> '; }
 ?>
	</tbody>
	</table>
