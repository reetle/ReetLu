...
$sql= "SELECT * FROM format LIMIT $start_from, $record_per_page "; 
...
<div class="item1">
	<div class="search_menu">
		<button onclick="window.location.href='format_add.php';">Lisa uus</button>
		<button onclick="#">Prindi</button>
		<button type="submit" form="form2" name="export"  >Ekspordi CSV</button>
<br><br>
<!--andmete eksport--> 
 <form method="post" action="format_export.php" id="form2" name="export">  </form>  
...
/*viimane otsing jääb otsing aknasse*/
 ...
 <!--filtreeringu tühistamiseks laeb lehe uuesti-->		
		<button onclick="window.location.href='format_data.php';">Tühista filtreering</button>
</div> </div>
<div class="item2">
 <!--raamatute menüü-->
<?php
...
if($column1 == 'include'){
$sql= "SELECT * FROM format where $column like '%$search%' " ;}
	
elseif($column1 == 'starts'){
$sql= "SELECT * FROM format where $column like ' " . $search . "%' " ;} // esitähe järgi 
	
elseif($column1 == 'ends'){
$sql= "SELECT * FROM format where $column like '%" . $search . "' " ;} //viimase tähe järgi

elseif($column1 == 'exactly'){
$sql= "SELECT * FROM format where $column like ' " . $search . " ' " ;} //täpselt  ei toimi 
}
...
<?php
    ...
    if($page > 1)
    {
     echo "<a href='format_data.php?page=".($page - 1)."'> << </a>";
    }
    for($i=$start_loop; $i<=$end_loop; $i++)
    {     
     echo "<a href='format_data.php?page=".$i."'>" .$i. "</a>";
    }
    if($page <= $end_loop)
    {
     echo "<a href='format_data.php?page=".($page + 1)."'> >> </a>";
     echo "<a href='format_data.php?page=".$total_pages."'> Lõppu </a>";
    }
  ?>  
  -->
 </div>
