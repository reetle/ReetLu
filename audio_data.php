...
$sql= "SELECT * FROM audio_video order by pealkiri LIMIT $start_from, $record_per_page "; 
...
<!--otsimise ja filtreerimise menüü-->
  <div class="col-lg-10" style="margin-bottom:33rem; "id="filter">
	<div class="search_menu">
		<button onclick="window.location.href='audio_add.php';">Lisa uus raamat</button>
 ...
 <form method="post" action="audio_export.php" id="form2">  
	...
	<form action=" audio_data.php" method="POST" class="vorm" >
		<select name="column">
            ...
            <option value="tuup">AV tüüp</option>
...
<div class="table-wrapper-scroll-y my-custom-scrollbar">
 ...
 <!-- filtreerib pealkirja järgi kasvavalt või kahanevalt, &#8693; lisab nooled -->	
        ...
        <th onclick="sortTable(2)">AV Tüüp &#8693;</th>     
...
if($column1 == 'include'){
$sql= "SELECT * FROM audio_video where $column like '%$search%' " ;}
	
elseif($column1 == 'starts'){
$sql= "SELECT * FROM audio_video where $column like ' " . $search . "%' " ;} // esitähe järgi 
	
elseif($column1 == 'ends'){
$sql= "SELECT * FROM audio_video where $column like '%" . $search . "' " ;} //viimase tähe järgi

elseif($column1 == 'exactly'){
$sql= "SELECT * FROM audio_video where $column like ' " . $search . " ' " ;} //täpselt  ei toimi 
}
...
$result = mysqli_query($conn, $sql) or die("error:".mysqli_error($conn));
  ...
  <tr>
	...
	<td>'.$row["marksona"].'</td>
	<td>'.$row["markused"].'</td>	
	<td><a href="borrow_audio.php?id='.$row["id"].'" class="btn  btn-sm">Laenuta</a></td>
	<td><a href="write_off_audio.php?id='.$row["id"].' "class="btn btn-sm">Kanna Maha</i></a></td>
  </tr> '; }
 ?>
	</tbody>
	</table>
...
<?php
	...
	$page_query = "SELECT *
	FROM audio_video ORDER BY 'pealkiri'";
    ...
    {     
     echo "<a href='audio_data.php?page=".$i."'>" .$i. "</a>";
    }
    if($page <= $end_loop)
    {
     echo "<a href='audio_data.php?page=".($page + 1)."'> >> </a>";
...
$(document).ready(function(){  
     $('#editable_table').Tabledit({
      url:'meth_library_action.php',
		...
		columns:{
       identifier:[0, "id"],
       editable:[[1, 'pealkiri'], [2, 'tuup'], [3, 'autor'], [4, 'aasta'],
