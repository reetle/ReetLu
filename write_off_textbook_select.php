...
$sql= "SELECT * FROM textbook where $column like ' " . $search . "%' " ;} // esitähe järgi 
	
elseif($column1 == 'ends'){
$sql= "SELECT * FROM textbook where $column like '%" . $search . "' " ;} //viimase tähe järgi

elseif($column1 == 'exactly'){
$sql= "SELECT * FROM textbook where $column like ' " . $search . " ' " ;} //täpselt  ei toimi 
}
...
$result = mysqli_query($conn, $sql) or die("error:".mysqli_error($conn));
  ...
  <tr>
	...
	<td>'.$row["marksona"].'</td>
	<td>'.$row["markused"].'</td>	
	<td><a href="write_off_textbook.php?id='.$row["id"].' "class="btn btn-sm">Kanna Maha</i></a>
  </tr> '; }
 ?>
	</tbody>
	</table>
