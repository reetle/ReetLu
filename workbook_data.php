...
<div class="item1">
	<div class="search_menu">
		<button onclick="window.location.href='workbook_add.php';">Lisa uus metoodiline kirjandus</button>
		<button type="submit" form="form2" name="export" class="export" >Ekspordi CSV</button>
		<button onclick="window.location.href='workbook.php';">Tühista filtreering</button>
<br><br>
<!--andmete eksport-->
 <form method="post" action="workbook.php" id="form2">  

                </form>  

	<!--filtreerimine tabeli pealkirjade järgi-->
	<form action="workbook.php" method="POST" class="vorm" >
		<select name="column">
...
if($column1 == 'include'){
$sql= "SELECT * FROM worktbook where $column like '%$search%' " ;}
...
$result = mysqli_query($conn, $sql) or die("error:".mysqli_error($conn));
  ...
  <tr>
	...
	<td><a href="write_off_workbook.php?id='.$row["id"].'">Kanna maha</a></td>
  </tr> '; }
 ?>
	</tbody>
	</table>
...
<?php
    ...
    if($page > 1)
    {
     echo "<a href='workbook.php?page=1'> Algusesse </a>";
     echo "<a href=workbook.php?page=".($page - 1)."'> << </a>";
    }
    for($i=$start_loop; $i<=$end_loop; $i++)
    {     
     echo "<a href='workbook.php?page=".$i."'>" .$i. "</a>";
    }
    if($page <= $end_loop)
    {
     echo "<a href='workbook.php?page=".($page + 1)."'> >> </a>";
     echo "<a href='workbook.php?page=".$total_pages."'> Lõppu </a>";
    }
  ?>  
 </div>
</div>	
