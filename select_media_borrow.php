...
//Raamat
	if ($select == 'book') {        
		header("Location: borrow_book_select.php"); 
}
//õpik
	elseif ($select == 'textbook' ) {	
	header("Location: borrow_textbook_select.php");   
}
//perioodika
	elseif ($select == 'periodicals' ) {	
	header("Location: borrow_periodicals_select.php");		
}
//audio-video
	elseif($select == 'audio-video' ) {	
	header("Location: borrow_audio_select.php");
}	

//töövihik
	elseif ($select == 'workbook' ) {
		header("Location: borrow_workbook_select.php");
}		
//metoodiline kirjandus
	elseif ($select == 'meth-library' ) {
		header("Location: borrow_meth_select.php");
