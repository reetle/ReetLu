...
<div class="row" id="borrow">
  ...
  <br>
  <p> Kas sa soovid antud perioodikat laenutada: </p>
 <?php
include_once("config.php");
$id = $_GET['id'];
$result = mysqli_query($conn, "SELECT * FROM periodicals WHERE id=$id") ;
   
 if (mysqli_num_rows($result) > 0) {
  while($row = mysqli_fetch_assoc($result)) {
    echo "Inventari nr: " . $row["id"]. "<br> " . $row["pealkiri"]. "<br>  " . $row["nr"]. "<br>";
  }
} 
?>
  </div> 
...
SELECT '$lugeja', periodicals.id, 'PE', periodicals.pealkiri, periodicals.autor, '$kogus', '$algus_kp'
